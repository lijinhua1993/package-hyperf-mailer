...
abstract class Mailable implements MailableInterface, CompressInterface, UnCompressInterface
{
    ...
    public function attachFromStorage(?string $adapter, string $path, ?string $name = null, array $options = []): self
    {
        $this->storageAttachments = \Hyperf\Collection\collect($this->storageAttachments)->push([
            'storage' => $adapter ?: ApplicationContext::getContainer()->get(ConfigInterface::class)->get('file.default'),
            'path'    => $path,
    ...
    {
        $queue = $queue ?: (property_exists($this,
            'queue') ? $this->queue : array_key_first(ApplicationContext::getContainer()->get(ConfigInterface::class)->get('async_queue')));
    ...
    protected function hasRecipient(
        ...
        $expected = $this->normalizeRecipient($address, $name);
